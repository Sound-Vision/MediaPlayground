...
//  NSLog(@"%s", __PRETTY_FUNCTION__);
  CVPixelBufferRef pixelBuffer = CMSampleBufferGetImageBuffer(sample_buffer);
  [self.videoRenderer renderVideoFrameWithPixelBuffer:pixelBuffer width:width height:height];
}
...
- (NSArray<NSDictionary *> *)functionInfoList {
  if (!_functionInfoList) {
    _functionInfoList = @[
      ...
      @{@"type" : @(VideoRenderActionTypeStartCapture), @"title" : @"停止采集"},
    ];
  }
